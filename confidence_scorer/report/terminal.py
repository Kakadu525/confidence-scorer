...
def render_terminal_report(result: PipelineResult, console: Console | None = None) -> None:
    ...
    table = Table(title="Разбивка по проверкам")
    ...
    pt_table.add_column("Функция")
    pt_table.add_column("Файл")
    pt_table.add_column("Статус")
    pt_table.add_column("Детали")
    for r in pt.results:
    ...
    if result.file_issues:
        issues_table = Table(title="Файлы, которые не удалось проанализировать")
        issues_table.add_column("Файл")
        issues_table.add_column("Язык")
        issues_table.add_column("Причина")
    ...
    if score.hard_fail_reasons:
        console.print(Panel("\n".join(score.hard_fail_reasons), title="Подтверждённые контрпримеры (hard fail)", style="bold red"))

    notable = result.semantic_result.notable_changes
    if notable:
        sem_table = Table(title="Semantic diff: заметные изменения поведения")
        sem_table.add_column("Функция")
        sem_table.add_column("Severity")
        sem_table.add_column("Описание")
    ...
    if is_panel:
        panel_table = Table(title="Второй AI-ревьюер: панель")
        panel_table.add_column("Ревьюер")
        panel_table.add_column("Вес", justify="right")
        panel_table.add_column("Оценка")
        panel_table.add_column("Статус")
    ...
    if review.issues:
        rv_table = Table(title="Второй AI-ревьюер: замечания")
        rv_table.add_column("Severity")
        rv_table.add_column("Файл")
        rv_table.add_column("Описание")
        if is_panel:
            rv_table.add_column("Кто нашёл")
    ...
    if review.summary:
        console.print(Panel(review.summary, title="Резюме второго ревьюера"))

    if score.notes:
        console.print(Panel("\n".join(f"• {n}" for n in score.notes), title="Примечания", style="dim"))
